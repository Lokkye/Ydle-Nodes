...
static uint8_t _atm_crc8_table[256] = { 0x00, 0x07, 0x0E, 0x09, 0x1C, 0x1B,
...
#elif defined(SIMPLETIMER_H)
	Timer1.setInterval(YDLE_FBIT, timerInterrupt);
#endif
}
			...
			if (crc_p != g_frameBuffer[i].crc) {
				...
				if (g_frameBuffer[i].type == YDLE_TYPE_CMD) {
					...
					if (g_frameBuffer[i].sender == g_sendFrameBuffer.receptor
							&& g_frameBuffer[i].receptor
									== g_sendFrameBuffer.sender) {
						wait_ack = 0;
						retry = 0;
						last_check = 0;
						return 1;
...
bool ydle::checkSignal(Frame_t *frame) {
	if (frame->sender == m_Config.IdMaster
			&& frame->receptor == m_Config.IdNode)
		return true;
	else
		return false;
}
	...
	if (pll_ramp >= 160) {
		...
		if (rx_active) {
			...
			if ((bit_count - 48) >= (rx_bytes_count * 16) && (length_ok == 1)) {
				...
				memcpy(&g_frameBuffer[pFrame], &g_m_receivedframe,
						sizeof(Frame_t));
				pFrame++;
...
void ydle::dataToFrame(Frame_t *frame, unsigned long destination,
		unsigned long sender, unsigned long type) {
	frame->sender = sender;
